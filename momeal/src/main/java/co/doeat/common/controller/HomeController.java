...
public class HomeController {
	...
	public String home(Model model, HttpSession session) {
		...
		model.addAttribute("userInfo", userService.userSelect(userId)); // 로그인 한 회원 정보 등록
		model.addAttribute("hmUserInfo", userService.userSelect(userId)); // 로그인 한 유저의 식단게시글, 팔로잉, 팔로워 수
		return "base/home";
	}
}
